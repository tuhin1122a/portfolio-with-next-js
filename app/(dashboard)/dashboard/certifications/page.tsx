...
export default function AdminCertificationsPage() {
  ...
  const handleSubmit = async (formData: Omit<Certification, '_id' | 'order'>, imageFile: File | null) => {
      try {
          let imagePath = editingCert?.imagePath || '';
          if (imageFile) {
              const uploadedPath = await uploadImage(imageFile);
              if (!uploadedPath) throw new Error("Image path not returned from upload.");
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => handleOpenDialog()}>Add New Certification</Button>
      </div>
