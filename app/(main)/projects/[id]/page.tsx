...
import { fallbackProjects } from "@/lib/fallbackData"; // ফলব্যাক ডেটা আলাদা ফাইলে রাখুন
import { Project } from "@/lib/models/project";
import { connectToDB } from "@/lib/mongodb";
...
// --- ডেটা ফেচিং ফাংশন (CENTRALIZED DATA FETCHING) ---
async function getProjectData(id: string) {
  try {
    ...
    return fallbackProjects[id as keyof typeof fallbackProjects] || null;
  } catch (error) {
    console.error("Error fetching project:", error);
    return fallbackProjects[id as keyof typeof fallbackProjects] || null;
  }
...
// --- মেটাডেটা জেনারেসন (METADATA GENERATION) ---
...
// --- মূল পেইজ কম্পোনেন্ট (MAIN PAGE COMPONENT) ---
export default async function ProjectPage({ params }: Props) {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden text-gray-800 dark:text-foreground transition-colors duration-300">
      <GlowBackground />
      <LightBackground />
      <main className="max-w-6xl mx-auto pt-24 pb-16 px-4 md:px-8">
        <Button variant="ghost" size="sm" asChild className="mb-8">
          <Link href="/#projects">
            <ArrowLeft className="mr-2 h-4 w-4" />
            Back to Projects
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          {/* Right Column */}
          <ProjectMedia title={project.title} mainImage={project.image} />
        </div>
